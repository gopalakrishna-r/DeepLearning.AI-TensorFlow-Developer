...
if __name__ == '__main__':
    ...
    with wandb.init(project="sunspots_forecaster", name='lr-rnn',
                    entity='goofygc316', resume=True) as run:
        config = run.config
        run.define_metric("mean_absolute_error", summary="min")

        train_dataset = batch_csv_dataset(x_train, window_size=config.window_size,
                                          batch_size=config.batch_size, shuffle_buffer_size=1000, cache=True)
        valid_dataset = batch_csv_dataset(x_valid, window_size=config.window_size,
                                          batch_size=config.batch_size, shuffle_buffer_size=1000, cache=True)
        ...
        history = model.fit(train_dataset,
                            epochs=100, callbacks=[WandbCallback()],
                            workers=10, use_multiprocessing=True)

        rnn_forecast = model_forecast(model, series[..., np.newaxis], config.window_size)
        rnn_forecast = rnn_forecast[split_time - config.window_size:-1, -1, 0]
        forecast__numpy = keras.metrics.mean_absolute_error(x_valid, rnn_forecast).numpy()
    ...
    api = wandb.Api()
    sweep = api.sweep("goofygc316/DeepLearning.AI-TensorFlow-Developer-TimeSeries/viatr2ae")
    runs = sorted(sweep.runs,
                  key=lambda run: run.summary.get("mean_absolute_error", 0))
    best_run = runs[2]
