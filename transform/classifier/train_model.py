...
ap.add_argument(
    ...
    type=str,
    default="plot.png",
    help="path to output loss/accuracy plt",
)
...
class_weights = {
    i: class_totals.max() / class_totals[i] for i in range(0, len(class_totals))
}

train_dataset = tf.data.Dataset.from_tensor_slices(train_paths)
train_dataset = (
    train_dataset.shuffle(len(train_paths))
...
val_dataset = (
    val_dataset.map(load_images, num_parallel_calls=tf.data.AUTOTUNE)
    .cache()
    .batch(batch_size=config.BS, num_parallel_calls=tf.data.AUTOTUNE)
    .prefetch(tf.data.AUTOTUNE)
)
...
test_dataset = (
    test_dataset.map(load_images, num_parallel_calls=tf.data.AUTOTUNE)
    .cache()
    .batch(batch_size=config.BS, num_parallel_calls=tf.data.AUTOTUNE)
    .prefetch(tf.data.AUTOTUNE)
)
...
model.compile(
    loss=keras.losses.BinaryCrossentropy(), optimizer=optimizer, metrics=["accuracy"]
)
