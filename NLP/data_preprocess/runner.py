...
from NLP.data_preprocess.feature_cleaning.writer import write_tf_records, parse_examples, build_feature_column, \
    save_to_multiple_files, mnist_dataset
...
if __name__ == '__main__':
    ...
    parser.add_argument('feature_cleaning', help='comma separated list of models to use.', default='1')
    ...
    if args.feature_cleaning:
        ...
        if run_type == 1:
            ...
            contents = [["When", "shall", "we", "three", "meet", "again", "?"],
            ...
            model.compile(optimizer=keras.optimizers.SGD(learning_rate=1e-3), metrics=['accuracy'], loss='mse')
            model.fit(dataset, steps_per_epoch=len(x_train) // batch_size, epochs=10, workers=16)
        elif run_type == 3:
            (x_train_full, y_train_full), (x_test_full, y_test_full) = datasets.fashion_mnist.load_data()
            x_train, x_valid = x_train_full[:5000], x_train_full[5000:]
            y_train, y_valid = y_train_full[:5000], y_train_full[5000:]

            train_set = tf.data.Dataset.from_tensor_slices((x_train, y_train)).shuffle(len(x_train), name="train")
            valid_set = tf.data.Dataset.from_tensor_slices((x_valid, y_valid), name="valid")
            test_set = tf.data.Dataset.from_tensor_slices((x_test_full, y_test_full), name='test')

            datasets = dict(train=train_set, valid=valid_set, test=test_set)

            dataset_filepaths = dict(
                map(lambda item: (item[0], save_to_multiple_files(f"fashion_mnist.{item[0]}", item[1])),
                    datasets.items()))

            tf_datasets = dict(
                map(lambda item: (item[0],
                                  mnist_dataset(item[1]) if item[0] != 'train' else mnist_dataset(item[1],
            ...
            logs = os.path.join(os.curdir, "tf_logs", "run_" + datetime.now().strftime("%Y%m%d_%H%M%S"))
            tensorboard_cb = keras.callbacks.TensorBoard(log_dir=logs, histogram_freq=1, profile_batch=10)

            model.fit(train_set, epochs=5, validation_data=tf_datasets.get('valid'),
                      workers=16,
