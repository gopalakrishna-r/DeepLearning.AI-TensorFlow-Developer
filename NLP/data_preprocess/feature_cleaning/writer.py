...
def save_to_multiple_files(name_prefix, data, n_parts=10):
    paths = ["{}.tfrecord-{:05d}-of-{:05d}".format(name_prefix, index, n_parts) for index in range(n_parts)]
...
def preprocess(tfrecord,
               feature_descriptions=dict(image=tf.io.FixedLenFeature([], tf.string, default_value=""),
...
def parse_examples(serial_examples, feature_descriptions):
    examples = tf.io.parse_example(serial_examples,
