...
if __name__ == "__main__":
    ...
    if args.rnn_model:
        ...
        if rnn_model == 1:
            ...
            padded, padded_testing, word_index = process_sequence(
                training_sentences, testing_sentences, vocab_size, max_length
            )
            ...
            write_embeddings(
                weights, vocab_size, lambda word_ind: reverse_word_index(word_ind)
            )
            ...
            if rnn_model_type == 1:
                ...
                model = build_text_embedding_model(
                    tokenizer.vocab_size, embedding_size, 6
                )
            elif rnn_model_type == 2:
                lstm_size = 64
                embedding_size = 64
                model = build_bi_embedding_model(
                    tokenizer.vocab_size, embedding_size, lstm_size, 64
                )
            elif rnn_model_type == 3:
                lstm_size = 64
                embedding_size = 64
                model = build_stacked_bi_embedding_model(
                    tokenizer.vocab_size, embedding_size, lstm_size, 64
                )
            ...
            train_data = train_data.padded_batch(
                BATCH_SIZE, tf.compat.v1.data.get_output_shapes(train_data)
            )
            test_data = test_data.padded_batch(
                BATCH_SIZE, tf.compat.v1.data.get_output_shapes(test_data)
            )

            history = model.fit(
                train_data, validation_data=test_data, epochs=num_epochs, workers=8
            )
            ...
            train_set = train_set.map(
                lambda x_batch, _: (table.lookup(x_batch), _)
            ).prefetch(1)
