...
def build_bi_embedding_model(vocab_size, embedding_size, max_sequence_len, lstm_size):
    return keras.Sequential([
        keras.layers.Embedding(vocab_size, embedding_size, input_length=max_sequence_len - 1),
...
def build_time_distributed_model(max_id):
    with slim.arg_scope([gru_layer], return_sequences=True, dropout=0.2, recurrent_dropout=0.2):
...
def build_stateful_time_distributed_model(max_id, batch_size=32):
    with slim.arg_scope([gru_layer], return_sequences=True, dropout=0.2, recurrent_dropout=0.2, stateful=True):
        return keras.models.Sequential([
