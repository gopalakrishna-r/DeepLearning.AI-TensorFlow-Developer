...
from data_loader import load_corpus_data, load_irish_songs, load_shakespeare_text, generate_shakespeare_dataset, \
    generate_shakespeare_dataset_stateful
from model import build_bi_embedding_model, build_tokenizer, build_time_distributed_model, \
    build_stateful_time_distributed_model, ResetStatesCallback
...
if __name__ == '__main__':
    ...
    if args.rnn_model:
        ...
        if rnn_model == 1:
            xs, ys, total_words, max_sequence_len, tokenizer = process_sequence(load_corpus_data())
            ...
            for _ in range(next_words):
                output_word = (seq([tokenizer.texts_to_sequences([seed_text])[0]])
                                       .map(lambda token: pad_sequences([token], maxlen=max_sequence_len - 1, padding='pre'))
                                       .map(lambda token: np.argmax(model.predict(token), axis=-1))
                                       .map(lambda predicted_word: (seq(tokenizer.word_index.items())
                                            .find(lambda key_index_tuple: key_index_tuple[1] == predicted_word)))
                                       .map(lambda key_index_tuple: key_index_tuple[0] if key_index_tuple else "")
                                       .fold_left("", lambda current, next_word: current + " " + next_word))
                seed_text += output_word
            print(seed_text)
        elif rnn_model == 2:
            xs, ys, total_words, max_sequence_len, tokenizer = process_sequence(load_irish_songs())
            ...
            for _ in range(next_words):
                output_word = (seq([tokenizer.texts_to_sequences([seed_text])[0]])
                               ...
                               .map(lambda predicted_word: (seq(tokenizer.word_index.items())
                                                            .find(
                    lambda key_index_tuple: key_index_tuple[1] == predicted_word)))
                               .map(lambda key_index_tuple: key_index_tuple[0] if key_index_tuple else "")
                               .fold_left("", lambda current, next_word: current + " " + next_word))
                seed_text += output_word
            print(seed_text)
            ...
            [encoded] = np.array(tokenizer.texts_to_sequences([shakespeare_txt])) - 1
            dataset = generate_shakespeare_dataset_stateful(encoded, dataset_size, max_id)
            ...
            model.compile(optimizer= 'adam', loss='sparse_categorical_crossentropy', metrics= ['accuracy'])
            print(model.summary())
            history = model.fit(dataset, epochs= 50, workers= 8, use_multiprocessing=True, callbacks=[ResetStatesCallback()])
