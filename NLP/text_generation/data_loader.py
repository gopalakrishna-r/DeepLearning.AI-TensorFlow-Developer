...
def load_irish_songs():
    keras.utils.get_file(fname='irish-lyrics-eof.txt',
                         origin='https://storage.googleapis.com/laurencemoroney-blog.appspot.com/irish-lyrics-eof.txt',
                         cache_dir='..\\data\\')
    txt = open(r'D:\codebase\DeepLearning.AI-TensorFlow-Developer\course_3\irish-lyrics-eof.txt').read()
    return txt
...
def generate_shakespeare_dataset(encoded_text, dataset_size, max_id, n_steps=100, batch_size=32):
...
def generate_shakespeare_dataset_stateful(encoded_text, dataset_size, max_id, n_steps=100, batch_size=32):
    train_size = dataset_size * 90 // 100
    window_length = n_steps + 1
